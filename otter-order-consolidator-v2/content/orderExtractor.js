class OrderExtractor {
  ...
  extractOrderFromPreview(orderRow) {
    try {
      ...
      // Update sizes for Urban Bowls in preview items
      previewItems.forEach(item => {
        if (item.name.toLowerCase().includes('urban bowl')) {
          item.size = 'urban';
        }
      });
        ...
        if (cachedOrder) {
          ...
          if (cachedOrder.items && cachedOrder.items.length > 0) {
            ...
            order.items = cachedOrder.items.map(cachedItem => ({
              ...
              size: cachedItem.size || 'no-size',
              quantity: cachedItem.quantity || 1,
              price: cachedItem.price || 0,
              category: this.categoryManager.categorizeItem(cachedItem.name),
      ...
      console.log(`Order ${orderNumber} - ${customerName}: wait time = ${waitTime}m, items: ${order.items.length}`);
  ...
  extractPreviewItems(orderRow) {
    ...
    try {
      ...
      // Create item objects - default all to 'no-size' unless explicitly found
      ...
      itemNames.forEach(name => {
        items.push({
          name: name,
          baseName: name,
          size: 'no-size', // Default everything to no-size
  ...
  extractDetailedItemData(itemRow) {
    ...
    while (nextSibling && !nextSibling.matches(this.orderSelectors.detailItemRow)) {
      ...
      if (nextSibling.matches && nextSibling.matches(this.orderSelectors.modifierSection)) {
        ...
        if (labelEl) {
          ...
          if (labelText.includes('Size Choice')) {
          ...
          } else if (labelText === 'Substitute Rice' && isUrbanBowl) {
            // For Urban Bowls, rice substitution acts like a size modifier
            if (values.length > 0) {
              riceSubstitution = values[0];
              // Use rice substitution as size indicator for Urban Bowls
            ...
            if (valueElements.length >= 2) {
              ...
              sideAdditions.push({
                ...
                quantity: 1,
                price: itemPrice,
                category: this.categoryManager.categorizeItem(itemName),
    ...
    if (filteredModifiers.length > 0) {
      ...
      if (isUrbanBowl) {
        finalSize = 'urban'; // Urban Bowls have their own size category
      } else if (size) {
  ...
  extractWaitTime(orderRow) {
    try {
      ...
      console.log('Extracting wait time from text:', timeText);
      
      // Extract number from formats like "• 19m", "• 1h 5m", "19m", etc.
      // First remove the bullet point if present
      const cleanTimeText = timeText.replace('•', '').trim();
